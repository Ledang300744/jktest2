...
var datatags=require("../models/tag");
var subjectsDb= require("../models/subjects");
      ...
      /**Accepted questions grouping by username and qID */
            ...
            .then((probSolved) => {
              ...
              for (var i = 0; i < probSolved.length; i++) {
                ...
                probSolvedObj[probSolved[i]._id.qID] =
                  1 + (probSolvedObj[probSolved[i]._id.qID] || 0);
              }
              ...
              function cmp(a, b) {
                ...
                if (probSolvedObj[a.qID] === null && probSolvedObj[b.qID] === null)
                  return -1;
                if (probSolvedObj[a.qID] && !probSolvedObj[b.qID]) return -1;
                if (!probSolvedObj[a.qID] && probSolved[b.qID]) return 1;
                return Number(probSolvedObj[a.qID]) > Number(probSolvedObj[b.qID])
                  ? -1
                  : 1;
              }
              ...
              datatags.find({}).then((dataTags)=>{
                data.sort(cmp);
                res.render("problem_set", { problems: data,dataTag:dataTags, solved: probSolvedObj});
              })
...
helper.displayProblem = async (req, res, next) => {
  ...
  problems
    ...
    .then((data) => {
      ...
      res.render("solution_submit", {langlist:lang, data: data });
    })
    .catch((err) => {
      console.log(err);
      next();
    ...
    .then((data) => {
      ...
      submission
        ...
        .then((user_questions) => {
          ...
          function cmp(a, b) {
            if (
              user_solved[a.username] === null &&
              user_solved[b.username] === null
            )
              return -1;
            if (user_solved[a.username] && !user_solved[b.username]) return -1;
            if (!user_solved[a.username] && user_questions[b.username])
              return 1;
            return Number(user_solved[a.username]) >
              Number(user_solved[b.username])
              ? -1
              : 1;
          }
          data.sort(cmp);
...
helper.submitSolutionTest=async (req, res, next) => {
  ...
  testcases.findOne({ qID: qID }, async (err, tc) => {
    ...
    var ratesuccess=successtestcase/lengthresults;
...
helper.submitSolution = async (req, res, next) => {
  ...
  testcases.findOne({ qID: qID }, async (err, tc) => {
    ...
    var ratesuccess=roundToTwo(successtestcase/lengthresults);
    //console.log("ratesuccess"+ratesuccess);
    //console.log("successtestcase"+successtestcase);
    var langName;
    ...
    problems.find({qID:qID}).then((dataskill)=>{
      var dataskillLength=dataskill[0].tags;
      console.log("LG"+dataskillLength.length);
      var idtags=[];
      for(var z=0;z<dataskillLength.length;z++){
    ...
    submission
    .find({qID:req.body.qID})
    .then(async(submitssionqid)=>{
      //console.log("submitssionqid"+submitssionqid);
      var submitssionqid_difficulty_sum =0;
      var difficulty_sum=0;
      for(var i=0; i<submitssionqid.length;i++){
      ...
      var updateDifficulty=roundToTwo(1-difficulty_sum);
      var editProblemDifficulty={
        difficultyAutoUpdate:updateDifficulty,
      }
      //console.log("updateDifficulty"+updateDifficulty);
      
        await problems.update({ qID:req.body.qID }, editProblemDifficulty)
        .then((val) => {
            ...
            if (check) {
              ...
              /* 
                  ...
                  // console.log("questions"+temp.solved_qID);
                   ...
                   for(var j=0;j<newSubmission.tc.length;j++){
                      //console.log("stt:"+newSubmission.tc[j].status);
                      if(newSubmission.tc[j].status === "Accepted" && current_problem_exists == false && check==true){
...
helper.chooseSubjects= async (req, res, next) => {
  subjectsDb.find({}).then((data)=>{
    res.render("page_choose_subjects",{data:data});
  })
}


helper.questionSubjects= async (req, res, next) => {
  var idSubjects=req.params.idSubject;
  var idChapters=req.params.idChapter;
  /**
  ...
  subjectsDb.find({_id:idSubjects}).then(async(data)=>{
    var dataUser=req.user ? req.user.username:"Guest" ;
    var arrayData=[];
    if(dataUser != "Guest"){
      submission.find({
        username:dataUser,
        ...
        for(var i=0;i<data1.length;i++){
          ...
          var arraySet = [...new Set(arrayData)];
          var dataUpdate={
            questionAccess:arraySet,
          }
          ///import vào db
        }
        await users.update({ username:dataUser}, dataUpdate)
        .then((val) => {
            console.log("EDITED: " + val);
        })
        .catch((err) => {
        ...
        users.find({ username:dataUser}).then(async(data2)=>{
          res.render("question_subjects",{data:data,data2:data2});
        })  
      })
    }
    else{
      res.render("question_subjects",{data:data});
    }
